...
*Moved 2026-09-17 to [cpu-offload.md](cpu-offload.md), which consolidates it with
TODO `cpu-offload`'s parallel log.* vLLM's offloaders skip EXL3 entirely — they
decide eligibility at construction time, when our parameters are still empty CPU
placeholders, and `process_weights_after_loading` then replaces those parameters
with objects the offloader never sees. `vllm_exl3_plugin/cpu_offload.py` registers
the finished tensors itself. Both causes, the rejected alternatives, the backend
choice and the measured cost are in that doc.
